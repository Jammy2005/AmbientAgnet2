...
print("Loaded API key:", os.getenv("LANGSMITH_API_KEY"))
...
# _SECRETS_DIR = Path("/Users/ahmadjamshaid/Desktop/DPS-Agentic/ambient_agents/credentials.json")
TOKEN_PATH = Path("/Users/ahmadjamshaid/Desktop/DPS-Agentic/ambient_agents/ambient_agents2/token.json")
...
def extract_message_part(payload):
    if payload.get("parts"):
        for part in payload["parts"]:
            print("MIME TYPE:", part.get("mimeType"))
        ...
        for part in payload["parts"]:
            return extract_message_part(part)  # recursive

    # Not multipart
    if payload.get("body", {}).get("data"):
        print("Found simple body")
...
async def ingest_email_to_langgraph(email_data, graph_name, url="http://127.0.0.1:2024"):
    ...
    #         "id": email_data["id"],
    #         "email_thread": email_data["thread_id"]
    #     }},
    #     multitask_strategy="rollback",
    # )
    
    # # ─── NEW: show exactly what LangGraph stored ───
    # run = await client.runs.get(thread_id, run.id, include_inputs=True)
    # print("\n📝  LangGraph received (server-side canonical form):")
    ...
    run = await client.runs.create(
        thread_id,
        graph_name,
        input=run_input,          # ← use the dict you just printed
        multitask_strategy="rollback",
    )
...
async def fetch_and_process_emails(args):
    ...
    try:
        ...
        for i, message_info in enumerate(messages):
            ...
            print("MIME Type:", message["payload"].get("mimeType"))
